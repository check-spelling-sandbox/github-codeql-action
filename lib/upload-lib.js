...
async function uploadPayload(payload, repositoryNwo, logger) {
    ...
    try {
        console.log("await client.request", "PUT /repos/:owner/:repo/code-scanning/analysis", {
...
async function waitForProcessing(repositoryNwo, sarifID, logger, options = {
    ...
    try {
        ...
        while (true) {
            ...
            try {
                console.log("await client.request", "GET /repos/:owner/:repo/code-scanning/sarifs/:sarif_id", {
                    owner: repositoryNwo.owner,
                    repo: repositoryNwo.repo,
                    sarif_id: sarifID,
                });
